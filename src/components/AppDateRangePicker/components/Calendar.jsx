...
class Calendar extends PureComponent {
  ...
  renderMonthAndYear(focusedDate, changeShownDate) {
    const styles = this.styles;
    const firstDate = addMonths(focusedDate, 0);
    const secondDate = addMonths(focusedDate, 1)
    return (
      <div onMouseUp={e => e.stopPropagation()} className={styles.monthAndYearWrapper}>
        <div className='rdrMonthAndYearWrapper__box'>
          <button
            type="button"
            className={classnames('rdrNextPrevButton--left', styles.nextPrevButton, styles.prevButton)}
            onClick={() => changeShownDate(-1, 'monthOffset')}>
            <IconLeft className='rdrNextPrevButton__icon'/>
          </button>
          <span className='rdrMonthAndYearWrapper__date'>{format(firstDate, 'MMMM')}</span>
          <button
            type="button"
            className={classnames('rdrNextPrevButton--right', styles.nextPrevButton, styles.nextButton)}
            onClick={() => changeShownDate(+1, 'monthOffset')}>
            <IconRight className='rdrNextPrevButton__icon'/>
          </button>
        </div>
        <div className='rdrMonthAndYearWrapper__box'>
          <button
          ...
          <span className='rdrMonthAndYearWrapper__date'>{format(secondDate, 'MMMM')}</span>
          <button
            type="button"
            className={classnames('rdrNextPrevButton--right', styles.nextPrevButton, styles.nextButton)}
            onClick={() => changeShownDate(+1, 'monthOffset')}>
